...
<?php foreach ($this->main['rows'] as $row):?>
	<div class="article" id="article_<?php echo $row['articleid'];?>">
		...
		<p><a href="?m=content&c=article&a=show&d=<?php echo $row['articleid'];?>">Read</a> | <a href="?m=content&c=user&a=write&d=<?php echo $row['articleid'];?>">Edit</a> | <a href="javascript:drop(<?php echo $row['articleid'];?>);void(0);">Drop</a></p>
	</div>
<?php endforeach;?>
...
function drop(id)
{
	$.post('<?php echo Common::url('content', 'user', 'do_rubbish', 'drop');?>', {"id":id}, function(r){
		if (r=='1') {
