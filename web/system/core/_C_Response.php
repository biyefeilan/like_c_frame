...
class _C_Response {
	...
	public static function output() {
		_C_App::assert ( self::$inited, 'Response does not be inited' );
	
		self::$content = self::content();
		
		if (is_array(self::$_config['replace'])) 
		{
			if (isset(self::$_config['replace']['string']) && !empty(self::$_config['replace']['string'])) 
			{
				self::$content = str_replace(array_keys(self::$_config['replace']['string']), self::$_config['replace']['string'], self::$content);
			}
			if (isset(self::$_config['replace']['pcre']) && !empty(self::$_config['replace']['pcre']))
			{
				foreach (self::$_config['replace']['pcre'] as $k=>$v)
				{
