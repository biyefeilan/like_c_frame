...
final class _C_Router 
{
	...
	public static function _getUri($mode)
	{
		...
		else
		{
			...
			$uri .= isset($_GET[$reg['class']['uri']]) ? $_GET[$reg['class']['uri']] . '/' : 'index/';
			$uri .= isset($_GET[$reg['method']['uri']]) ? $_GET[$reg['method']['uri']] .'/' : 'index/';
...
//----------------------------------------------------------------
	...
	{	
		...
		$suflen = strlen(self::URI_SUFFIX);
		
		if (strripos($uri, self::URI_SUFFIX) === strlen($uri) - $suflen)
		{
