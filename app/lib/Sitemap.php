...
final class Sitemap {
	...
	public static function create($check = true)
	{
		...
		if ($check && file_exists(self::$_filename) &&  $lastmod == date('Y-m-d', filemtime(self::$_filename)+3*24*3600))
		{
			return ;
		}
		else 
		...
		self::$_xml[] = '<urlset>';
		self::$_xml = array_merge(self::$_xml, self::url(self::$_domain, $lastmod, 'daily', '1.0'));
		foreach ($info as $c => $table)
		{
			$loc = str_replace(array('&'), array('&amp'), substr(self::$_domain, 0, -1).Url::mkurl($c, 'index'));
			self::$_xml = array_merge(self::$_xml, self::url($loc, $lastmod, 'weekly', '0.9'));
			foreach (DB::findAll($table, null, array('id'=>'DESC'), 'id', self::URL_MAX>0 ? self::URL_MAX : null) as $row)
			{
				...
				self::$_xml = array_merge(self::$_xml, self::url($loc, $lastmod, 'weekly', '0.7'));
			}
		}
